...
module GitlabWebHook
  describe GetJenkinsProjects do
    context "when fetching projects by request details" do
      let(:details) { double(RequestDetails, :branch => "master", :repository_url => "http://localhost/peronospora") }
      ...
      it "finds projects matching details" do
        expect(not_matching_project).to receive(:matches?).with(details.repository_url, details.branch, false).and_return(false)
        expect(matching_project).to receive(:matches?).with(details.repository_url, details.branch, false).and_return(true)
      ...
      it "finds projects matching details exactly" do
        expect(not_matching_project).to receive(:matches?).with(details.repository_url, details.branch, true).and_return(false)
        expect(matching_project).to receive(:matches?).with(details.repository_url, details.branch, true).and_return(true)
    ...
    context "when fetching master project matching request details" do
      ...
      it "finds project matching details and master branch" do
        expect(not_matching_project).to receive(:matches?).with(details.repository_url, Settings.any_branch_pattern).and_return(true)
        expect(not_matching_project).to receive(:matches?).with(details.repository_url, Settings.master_branch, true).and_return(false)
        expect(matching_project).to receive(:matches?).with(details.repository_url, Settings.any_branch_pattern).and_return(true)
        expect(matching_project).to receive(:matches?).with(details.repository_url, Settings.master_branch, true).and_return(true)
      ...
      it "finds first projects matching details and any non master branch" do
        ...
        expect(not_matching_project).to receive(:matches?).with(details.repository_url, Settings.master_branch, true).and_return(false)
        expect(matching_project).to receive(:matches?).with(details.repository_url, Settings.any_branch_pattern).and_return(true)
        expect(matching_project).to receive(:matches?).with(details.repository_url, Settings.master_branch, true).and_return(false)

RSpec.shared_context 'projects' do
  let(:repo_url) { 'http://example.com/diaspora.git' }
  ...
  let(:repo_url3) { 'git@example.com:discourse.git' }
  let(:repository3) { double('RemoteConfig', name: 'origin', getURIs: [URIish.new(repo_url3)], getFetchRefSpecs: [refspec]) }
  let(:scm3) { double(GitSCM, repositories: [repository3], branches: [BranchSpec.new('origin/master')], buildChooser: build_chooser) }

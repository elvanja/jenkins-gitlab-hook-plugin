  ...
  describe GetBuildActions do
    ...
    context "with project parameters" do
      ...
      before(:each) do
        ...
        details.stub(:repository_id).and_return('15')
      end
      ...
      it "keeps other parameters" do
        ...
        actions = subject.with(project, details)
        expect(actions.parameters.size).to eq(3)
      ...
      it "adds the repository_id parameter from details" do
        project.stub(:get_default_parameters).and_return([branch_parameter, other_parameter])

        actions = subject.with(project, details)
        expect(actions.parameters.size).to eq(3)
        repository_id = actions.parameters.find { |parameter| parameter.name == 'repository_id'}
        expect(repository_id).not_to be_nil
        expect(repository_id.value).to eq('15')
      end
